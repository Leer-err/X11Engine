...
PhysicsSystem::PhysicsSystem(float fixed_step)
    : fixed_step(fixed_step), accumulated_time(0) {}

PhysicsSystem::~PhysicsSystem() {}

void PhysicsSystem::simulate(World& world, float delta_time) {
    accumulated_time += delta_time;

    while (accumulated_time > fixed_step) {
        Physics::Scene::get().simulate(fixed_step);
