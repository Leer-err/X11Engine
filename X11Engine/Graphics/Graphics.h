...
class Graphics {
   ...
   public:
    ...
    ComPtr<ID3D11Buffer> CreateVertexBuffer(UINT size, bool dynamic,
                                            bool streamout,
                                            const void *data) const;
    ComPtr<ID3D11Buffer> CreateIndexBuffer(UINT size, bool dynamic,
                                           const void *data) const;
    bool UpdateConstantBuffer(ComPtr<ID3D11Buffer> buffer, const void *data) const;
    bool UpdateBuffer(ComPtr<ID3D11Buffer> buf, const void *data, size_t size) const;
    ...
    ComPtr<ID3D11ShaderResourceView> CreateTexture2DSRV(
        ComPtr<ID3D11Resource> res, DXGI_FORMAT format = DXGI_FORMAT_UNKNOWN) const;
   ...
   private:
    ...
    ComPtr<ID3D11RenderTargetView> CreateTexture2DRTV(ComPtr<ID3D11Resource> res,
                                               UINT mip) const;
    ComPtr<ID3D11Texture2D> CreateDepthStencil2D(DXGI_FORMAT format, bool GPUReadable,
                                          int width, int height) const;
    ComPtr<ID3D11DepthStencilView> CreateTexture2DDSV(ComPtr<ID3D11Resource> res,
                                               UINT mip) const;
