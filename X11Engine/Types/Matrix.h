...
struct matrix {
	...
	inline operator DirectX::XMMATRIX() const { return DirectX::XMLoadFloat4x4(&this->matr); }
...
inline matrix PerspectiveProjectionMatrix(const float aspectRatio, const float fov, const float farZ, const float nearZ) {
	return DirectX::XMMatrixPerspectiveFovLH(fov, aspectRatio, nearZ, farZ);
}

inline matrix __vectorcall LookToMatrix(const vector3& eyePosition, const vector3& eyeDirection, const vector3& upDirection)
{
