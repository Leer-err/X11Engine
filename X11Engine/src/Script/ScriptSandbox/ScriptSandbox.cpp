...
ScriptSandbox::TableBuilder::TableBuilder(lua_State* state,
                                          const std::string& name)
    : state(state), name(name) {
    lua_newtable(state);
}

ScriptSandbox::TableBuilder::~TableBuilder() {
...
ScriptSandbox::TableBuilder ScriptSandbox::createTable(const std::string name) {
    return TableBuilder(wrapper->state, name.c_str());
}
