...
class ChunkAllocator {
   public:
    ...
    class iterator : public std::iterator<std::forward_iterator_tag, T> {
       public:
        iterator(typename std::list<MemoryChunk*>::iterator begin,
        ...
        inline bool operator==(const iterator& other) const {
            return (m_currentChunk == other.m_currentChunk) &&
    ...
    ChunkAllocator() {
        PoolAllocator* allocator = new PoolAllocator(
            sizeof(T), alignof(T), allocSize,
            GlobalAllocator::get()->allocate(allocSize, alignof(T)));
