...
class ComponentManager {
    ...
    template <class T>
    class ComponentContainer : public Memory::ChunkAllocator<T, CHUNK_SIZE>,
   ...
   public:
    ...
    T* const AddComponent(const EntityId entityId, Args&&... args) {
        ...
        if (componentList == m_lookupTable.end()) {
            vector<IComponent*> newComponents(Helper<IComponent>::Get(),
                                              nullptr);
   ...
   protected:
    ...
    ComponentContainer<T>* GetComponentContainer() {
        ...
        if (containerPair == m_componentContainers.end()) {
            ...
            container =
                dynamic_cast<ComponentContainer<T>*>(containerPair->second);
        }
        return container;
    }
   ...
   private:
    ...
    unordered_map<uint32_t, std::vector<IComponent*>> m_lookupTable;
};
}  // namespace ECS